...
def get_state(env, idx=0, ind_episode=1., epsi= 0.02):
    ...
    # link_stability_time = env.link_time[idx]
...
with g.as_default():
    ...
    optim = tf.train.RMSPropOptimizer(learning_rate=1E-3, momentum=0.95, epsilon=0.01).minimize(g_loss)
...
for ind_agent in range(numUsers):  # initialize agents
    ...
    agent = Agent(memory_entry_size=n_input)
    agents.append(agent)
...
if IS_TRAIN:
    for i_episode in range(n_episode):
        ...
        for i_step in range(n_step_per_episode):
            ...
            # env.renew_channel()
            # env.compute_link_timing(action_temp)
            ...
            for i in range(numUsers):
                ...
                agents[i].memory.add(state_old, state_new, train_reward, action)  # add entry to this agent's memory
                    ...
                    # training this agent
                if time_step % mini_batch_step == mini_batch_step-1: ### ennd of episode
                    loss_val_batch = q_learning_mini_batch(agents[i], sesses[i])
                    record_loss.append(loss_val_batch)
                    if i == 0:
                        print('step:', time_step, 'agent',i, 'loss', loss_val_batch)

                if time_step % target_update_step == target_update_step-1:
                    update_target_q_network(sesses[i])
                    if i == 0:
...
if IS_TEST:
    ...
    data_rand_path = os.path.join(current_dir, "model/" + label + '/data_rand.mat')
    scipy.io.savemat(data_rand_path, {'data_rand': data_rand})
