...
class Environmnet:
    ...
    def __init__(self, numUsers, width, length, vlb, vup, distancelb, distanceup, delta_theta):
        ...
        self.power = 1
        self.delta_tau = 0.1  ## Agen can select antenna every delta_tau
        ...
        self.interference = np.zeros((self.numUsers, self.Antenna.bWNumb))
    ...
    def compute_reward(self, action):
        ...
        penalty = np.zeros((self.numUsers,1))
        ...
        for rx in range (self.numUsers):
            ...
            penalty[rx] = max( 1 -(self.bAtime[rx]/ min(self.delta_tau, self.link_time[rx])), 0)
            for tx in range (self.numUsers):
                if tx == rx:
                    intereference[tx] = 0
            ...
            # rewards[rx] = self.channel.bandwidth * np.log2(1 + sinr[rx]) * penalty[rx]/10E7
            rate[rx] = self.channel.bandwidth * np.log2(1 + sinr[rx]) * penalty[rx]
            self.data[rx] -= rate[rx] * min(self.delta_tau, self.link_time[rx])
            rewards[rx] = rate[rx] * min(self.delta_tau, self.link_time[rx])/self.data_size ### Amunt of data sent in each time step is the individual reward of the agent
    ...
    def compute_reward_test_sarl(self, action):
        self.compute_link_timing(action)
        signal = np.zeros((self.numUsers,1))
        sinr = np.zeros((self.numUsers,1))
        rate = np.zeros((self.numUsers,1))
        ...
        for rx in range (self.numUsers):
            ...
            sinr[rx] = signal[rx]/ sumInt
            rate[rx] = self.channel.bandwidth * np.log2(1 + sinr[rx]) * penalty[rx]
            self.data_sarl[rx] -= rate[rx] * min(self.delta_tau, self.link_time[rx])
            self.individual_time_limit[rx] -= self.delta_tau
        self.data_sarl[self.data_rand_sarl < 0] = 0
        # rate[self.data_rand == 0] = 1
        self.active_links_sarl[np.multiply(self.active_links_sarl, self.data_sarl <= 0)] = 0
        return rate
    ...
    def act_for_testing_sarl(self, actions):
        ...
        action_temp = actions.copy()
        rate = self.compute_reward_test_sarl(action_temp)
        D2D_success = 1 - np.sum(self.active_links_sarl) / (self.numUsers)  # V2V success rates
    ...
    def new_game(self, numUsers = 0):
        ...
        self.data_sarl = self.data_size * np.ones((self.numUsers,1))
        self.individual_time_limit_sarl = self.horizon * np.ones((self.numUsers,1))
        self.active_links_sarl = np.ones((self.numUsers,1),dtype ='bool')
